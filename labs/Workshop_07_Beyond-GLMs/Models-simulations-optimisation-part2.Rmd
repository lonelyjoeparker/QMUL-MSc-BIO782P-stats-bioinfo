...
Software today; you'll need:
 ...
 - `FigTree` and `Tracer`, both part of the BEAST package at http://beast.bio.ed.ac.uk. The FigTree archive is at http://tree.bio.ed.ac.uk/download.html?name=figtree&id=96&num=3 and the Tracer archive is at http://tree.bio.ed.ac.uk/download.html?name=tracer&id=88&num=3.
...
To use RAxML you will need to make sure the paths to `raxmlHPC` and `mc.paml` are both fully specified, or RAxML won't be able to find it. You can either do this by putting them in the same directory (not great), making a zillion copies of both every time you change projects (....) or creating an alias. An alias is a pointer which tells the shell 'when I ask for this, look for it here', e.g.

```{bash eval=F}
alias myraxmllocation=/Applications/Phylogenetics/RAxML/RAxML-7.2.8-ALPHA/raxmlHPC
cd ~
myraxmllocation -v
#
...
### Downloading and building FigTree and Tracer

 1) First download the *.tgz files listed above. These are compressed archives (directories). Unpack them somewhere (doesn't matter where). 
 2) Open a new terminal window and `cd` into the folder you extracted. Look for a `lib` folder and enter it.
 3) FigTree and Tracer are both supplied as Java executables. To run these we call `java`, telling it to expect an executable, like this:
 `java -jar Tracer.jar`
 Where the `-jar` option just means 'I'm going to give you an executable, look here for instructions. The process is the same for FigTree.
...
```bash
raxml -s mc.paml -n model-gtrgammaiN20 -m GTRGAMMAI -p 33399
# where:
...
# -p = random seed. The computer uses this to 'create' a list of random numbers.
